...
class Simplex:
    ...
    def __init__(self):
        ...
        self.certificate = []
        self.is_max = True
        # Não estava sendo utilizado, alterei para adaptar para os casos em que não apareciam variáveis na função objetivo
        self.obj = 0
        self.solution = []
        self.variable_set = {}
    ...
    def build_tableu(self, coeffs, bs, flag):
        ...
        self.num_vals = 0
        for k in self.variable_set.keys():
            if not k.startswith("aux"):
                self.num_vals +=1
        self.bs = bs
        # Para adaptar para os casos em que não apareciam variáveis na função objetivo
        bs = np.hstack(([self.obj], bs))
    ...
    def define_exit_line(self, entry):
        ...
        for line in range(1, len(self.tableu)):
            if self.tableu[line][entry] > 0:
                ...
                res[line] = div
        if len(res) == 0:
            # Precisei alterar para o certificado não ser vazio, mas que estivesse correto, quando não havia variáveis na função objetivo
            # e para corrigir um bug que apareceu ao fazer as alterações
            # print(self.tableu[:, entry])
            self.certificate = np.zeros(len(self.tableu[0]) - 1 - len(self.bs))
            if len(self.obj_fun_vars) != 0:
                for idx in range(len(self.bs), len(self.tableu[0]) - 1):
                    pos = self.tableu[0][idx] == 0
                    if pos:
                        list_col = list(self.tableu[:, idx])
                        if 1 in list_col and sum(list_col) == 1:
                            self.certificate[idx-len(self.bs)] = -1 * self.tableu[list_col.index(1), entry]
                self.certificate[entry-len(self.bs)] = 1
                self.unbounded = True
            self.certificate = self.certificate[:len(self.obj_fun_vars)]
            if round(self.certificate[0]) == 0 and len(self.certificate) == 1:
                self.certificate = [1.0]
            return -1
    ...
    def get_solution(self):
        ...
        if len(self.solution) == 0:
            # Para a solução não ser vazia no caso da variável da função objetivo não estar presente nas restrições
            self.solution = "".join(['0' for var in list(self.variable_set.keys()) if var[0:3] != 'aux'])
        if self.unbounded:
            solution += "ilimitado\n"
...
class Parser:
    ...
    operations = ["+", "-", "*", "/"]
    # Alterações pequenas nas regex para adequá-las ao formato das instâncias de teste
        ...
        if len(variable_w_coeff_match) >= 1:        
            ...
            for var in variable_w_coeff_match:
                ...
                if self.variable_set[variable][0] == "mul":
                    ...
                    count += coeff * self.variable_set[variable][1]
    ...
    def transform_to_canonical_form(self, line):
        ...
        constraint_symbols_match = re.findall(self.constraint_symbols_pattern, line)
        # Corrigir bug que apareceu ao fazer alterações
        b = self.extract_b_value(line)
        flag = False
        if not (len(variable_w_coeff_match) <= 1 and constraint_symbols_match[0][0] == ">=" and b == 0):
        ...
        if len(variable_w_coeff_match) <= 1:
            ...
            if constraint_symbols_match[0][0] == "<=":
                ...
                self.variable_set["aux{num}".format(num = self.num_aux)] = ("keep", 1)
                self.num_aux += 1
        # Para lidar com o caso da função objetivo sem variáveis
        if not flag:
    ...
    def correct_fun_obj(self):
        # Para lidar com a função objetivo sem variáveis
        if len(self.list_of_coeffs[0]) == 0:
            self.list_of_coeffs[0] = np.zeros(len(self.list_of_coeffs[1]))
        if self.is_max:
...
with open(sys.argv[1]) as input:
    ...
    if len(list_of_states[0]) == 0:
        parser.list_of_coeffs.append([])
        simplex.obj = float(" ".join(obj_function_raw[1:]))

    for line in constraints:
        parser.findall_variables(line)
        state = parser.transform_to_canonical_form(line)
        # Lidar com função objetivo sem variáveis
        if state != []:
...
for (state, b) in zip(list_of_states[1:], parser.list_of_b):
    # Corrigir bug que surgia quando as restrições eram passadas em uma determinada ordem
    if not (len(state) == 1 and b == 0):
